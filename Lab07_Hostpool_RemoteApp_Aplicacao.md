# Lab 07 — Host Pool de Aplicação (RemoteApp) publicando o Notepad++ da imagem customizada
...
> **Dependência:** **Lab 03** (domínio `avdlab.local` ativo, DNS na VNet, Entra Connect) **e Lab 06** (imagem `win11-avd-prd-cin` v1.0.0 no gallery `galavdprdcin001`, **com o Notepad++ já instalado**).
...
flowchart LR
    ...
    AG -->|"publica"| APP["📝 Notepad++<br/>(app isolado)"]
...
> **Leitura:** reutilizamos a **imagem customizada do Lab 06** (que já tem o Notepad++ instalado por máquina) para subir **1 host**, ingressado no `avdlab.local`. Em vez de publicar a **área de trabalho inteira**, publicamos **apenas o aplicativo** Notepad++ como **RemoteApp** — ele abre numa janela isolada, como se fosse local.
...
| **Objetivo** | Criar um **host pool Pooled do tipo RemoteApp**, com **1 host** provisionado a partir da **imagem customizada** (Lab 06) e ingressado no AD DS; **publicar o Notepad++** como RemoteApp; atribuir o usuário e validar no **Windows App**. |
...
| **Entrega** | Notepad++ publicado como **RemoteApp**, aparecendo no Windows App do usuário e abrindo como **app isolado** (sem desktop completo). |
...
- **RemoteApp (este lab):** o usuário recebe **apenas o aplicativo publicado** (ex.: Notepad++), numa **janela própria**, integrada ao desktop local. Mais leve, mais focado e reduz a superfície exposta (o usuário não vê o Windows do host).
...
3. **Virtual Machines → Add virtual machines = Yes:**
   ...
   - **Image:** clique em **See all images / Shared Image** → aba **Shared Images** → selecione:
     - **Gallery:** `galavdprdcin001` → **Image definition:** `win11-avd-prd-cin` → **Version:** `1.0.0` (a imagem do Lab 06, **com o Notepad++ já instalado**).
...
3. No workspace, deve aparecer o **ícone do Notepad++** (`Notepad++`) — **e não** um desktop completo.
4. Clique nele → o **Notepad++ abre numa janela própria**, integrada ao seu desktop local (sem a área de trabalho do host).
...
- [ ] Notepad++ publicado como aplicação no Application Group RemoteApp.
- [ ] Usuário atribuído vê **o app Notepad++ (não um desktop)** no Windows App.
- [ ] Notepad++ **abre como janela isolada** ao clicar.
...
| App não publica | **Application groups → Applications** | Notepad++ listado; caminho/menu Iniciar corretos |
